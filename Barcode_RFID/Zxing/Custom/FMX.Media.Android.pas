...
uses
  System.Classes, System.RTLConsts, System.Threading, System.Math,
  System.Permissions, System.SysUtils, System.SyncObjs,
  System.Generics.Collections, Androidapi.Bitmap, Androidapi.JNI.Hardware,
  Androidapi.Gles, Androidapi.JNI.Os,
  Androidapi.Helpers, FMX.Consts, FMX.Types, FMX.Surfaces, FMX.Graphics,
  FMX.Helpers.Android, FMX.Forms, FMX.Platform;
...
begin
  if PermissionsService.IsPermissionGranted
    (JStringToString(TJManifest_permission.JavaClass.RECORD_AUDIO)) then
  begin
...
begin
  ...
  SharedSurface.SetSize(SharedBufferSize.X, SharedBufferSize.Y,
    TPixelFormat.RGBA);
...
procedure TAndroidVideoCaptureDevice.ConvertYuvToBitmapSurfaceHandler
  (Sender: TObject);
  ...
  begin
    QueueSection.Acquire;
    try
      Image := TJYuvImage.JavaClass.init(SharedBuffer, SharedBufferFormat,
        PreviewBufferSize.X, PreviewBufferSize.Y, nil);
    finally
      QueueSection.release;
    end;
    ...
    Result := TJBitmapFactory.JavaClass.decodeByteArray(Stream.toByteArray, 0,
      Stream.Size, LoadOptions);
  end;
  ...
  begin
    ...
    TempBitmap := TJBitmap.JavaClass.createScaledBitmap(ABitmap, ScaledSize.X,
      ScaledSize.Y, True);
    try
      Matrix := TJMatrix.JavaClass.init;
      Matrix.postRotate(ManualBitmapRotation);

      RotatedBitmap := TJBitmap.JavaClass.createBitmap(TempBitmap, 0, 0,
...
begin
  if (FPreviewConversionTask <> nil) and
    (FPreviewConversionTask.Status in [TTaskStatus.Running,
    TTaskStatus.WaitingToRun]) then
    Exit;

  FPreviewConversionTask := TTask.Create(Self,
    ConvertYuvToBitmapSurfaceHandler).start;
end;
  ...
  begin
    ...
    try
      if Dest.Canvas.BeginScene then
        try
          Dest.Canvas.DrawBitmap(Source, TRectF.Create(0, 0, Source.width,
            Source.height), TRectF.Create(0, 0, Dest.width, Dest.height), 1);
        finally
...
begin
  ...
  try
    ...
    begin
      if (SharedSurface.width = ABitmap.width) and
        (SharedSurface.height = ABitmap.height) then
        ...
        if ABitmap.Map(TMapAccess.Write, BiData) then
          try
            for I := 0 to SharedBufferSize.Y - 1 do
              Move(SharedSurface.Scanline[I]^, BiData.GetScanline(I)^,
                SharedBufferSize.X * SharedSurface.BytesPerPixel);
          finally
...
begin
  if (Control <> nil) and (Control.Root <> nil) and
